 ...
 * will return chemical potential or fugacity that is constant or as a function of <b>x</b>,<b>y</b>
 * and <b>z</b>.
 *
 * ParticleType class holds information on the particle's quark content @f$ N_i @f$ (with index
 * @f$ i = q, \bar q, s, \bar s, c, \bar c @f$) and quantum numbers @f$ B @f$, @f$ I_3 @f$, @f$ S
 * @f$ and @f$ C @f$. <br /> Chemical potential is calculated as follows
 * @f[ \mu = \mu_B B + \mu_{I_3} I_3 + \mu_S S + \mu_C C@f]
 * and the fugacity
 * @f[ \Upsilon = \lambda_{I_3} \,\gamma_q^{N_q + N_{\bar q}} \,\lambda_q^{N_q - N_{\bar q}}
 * \,\gamma_s^{N_s + N_{\bar s}} \,\lambda_s^{N_s - N_{\bar s}} \,\gamma_c^{N_c + N_{\bar c}}
 * \,\lambda_c^{N_c - N_{\bar c}} @f]
 *
 ...
 * @retval 3 fugacities @f$ \lambda_j(x, y, z) @f$ and phase space occupancies @f$ \gamma_k(x, y, z)
 * @f$
 ...
 *
 * @fn double Chemistry::GetChemicalPotential(ParticleType* aPartType, double aX, double aY, double
 * aZ) const
 ...
 * @param [in] aMuC strange chemical potential
 *
 * @fn void Chemistry::SetChemistry(double aLambdaQ, double aLambdaI, double aLambdaS, double
 * aLambdaC, double aGammaQ, double aGammaS, double aGammaC)
 ...
 *
 *
 * @fn void Chemistry::SetChemistry(Vector3D* aLambdaQ, Vector3D* aLambdaI, Vector3D* aLambdaS,
 * Vector3D* aLambdaC, Vector3D* aGammaQ, Vector3D* aGammaS, Vector3D* aGammaC)
