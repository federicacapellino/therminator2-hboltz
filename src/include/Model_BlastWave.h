...
class Model_BlastWave : public Model {
public:
  Model_BlastWave();
 ...
 * @f{eqnarray}{
 *   I			&=& \frac{2 s+1}{\left(2\pi\right)^3} \,d\Sigma_\mu p^\mu \left[ \exp \left(
 * \frac{p_\mu u^\mu - \mu_{ch}}{T}\right) \pm 1\right]^{-1} p_T \,dp_T \,d\phi_p \,dy \nonumber \\
 *   \nonumber\\ \nonumber\\
 *   d\Sigma_\mu p^\mu	&=& m_T \cosh(\eta_\parallel - y) \,\tau \,\rho \,d\rho \,d\phi
 * \,d\eta_\parallel \nonumber \nonumber \\ p_\mu u^\mu	&=& \frac{1}{\sqrt{1-v_T^2}} \left[ m_T
 * \cosh(\eta_\parallel - y) - v_T \,p_T \cos(\phi - \phi_p)\right] \nonumber
 * @f}
