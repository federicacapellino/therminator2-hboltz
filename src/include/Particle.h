...
class Particle : public ParticleCoor {
public:
  Particle();
  ...
  void SetParticlePX(double aPe, double aPx, double aPy, double aPz, double aXt, double aXx,
                     double aXy, double aXz, double aWeight = 1.0);
  void SetParticlePX(double aPe, double aPx, double aPy, double aPz, double aXt, double aXx,
                     double aXy, double aXz, Particle* aFather);
  const char* MakeTEXTEntry();
  static void ZeroEID();

private:
  static std::atomic<long> EIDi;
 ...
 *
 * Class has information about the PDG Monte-Carlo number of the ParticleType, father particle and
 * the original primordial particle To back trace the whole decay cascade Particle stores the unique
 * particle number of the particle in event and its fathers. Particle also holds information if the
 ...
 *
 * @fn void Particle::SetParticlePX(double aPe, double aPx, double aPy, double aPz, double aXt,
 * double aXx, double aXy, double aXz, Particle* aFather=0)
