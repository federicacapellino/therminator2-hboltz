...
Event::Event() : mPartDB(0), mInteg(0), mDistribution(0) {
  mMultiplicities.clear();
  Reset();
}

Event::Event(ParticleDB* aDB, Integrator* aInteg) : mPartDB(aDB), mInteg(aInteg), mDistribution(0) {
  mMultiplicities.clear();
...
void Event::GeneratePrimordials() {
  ...
  for (int tIter = 0; tIter < mPartDB->GetParticleTypeCount(); tIter++) {
    if (!strstr(mPartDB->GetParticleType(tIter)->GetName(), "gam000zer")) {
      ...
      mInteg->GenerateParticles(mPartDB->GetParticleType(tIter), mMultiplicities[tIter],
                                &mParticles);
...
void Event::DecayParticles() {
  ...
  // as new particles are added from decays the end() of the list moves until all particles had
  // decayed
  do {
    tFatherType = tIter->GetParticleType();
    // if not stable or stable but has a decay table with at least one decay channel
    if ((tFatherType->GetGamma() >= 0.0) && (tFatherType->GetTable()) &&
        (tFatherType->GetTable()->GetChannelCount() + 1 > 0))
...
void Event::SetDistribution(const std::string& name) {
  if (name == "NegativeBinomial") {
    ...
    PRINT_MESSAGE(
        "Event::SetDistribution: bad selection in switch. Setting the distribution to Poisson.");
    mDistribution = 0;
  }
}
