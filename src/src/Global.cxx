...
void ReadParameters() {
  ...
  try {
    ...
    tModelINI = sMainConfig->GetParameter("FreezeOutDir");
    tModelINI.Prepend("./");
    tModel = sMainConfig->GetParameter("FreezeOutModel");
    if (tModel == "KrakowSFO") {
      sModel = 0;
      tModelINI += "krakow.ini";
    } else if (tModel == "BlastWave") {
      sModel = 1;
  ...
  try {
    ...
    PRINT_MESSAGE("<therm2_events::ReadParameters>\tUsing custom Freeze-Out-Model INI file "
                  << sModelINI);
  } catch (TString tError) {
    sModelINI = tModelINI;
  }
...
void CheckSHARE(ParticleDB* aPartDB) {
  ...
  PRINT_DEBUG_2("<therm2_events::ParserCheck>\tRead " << aPartDB->GetParticleTypeCount()
                                                      << " particle types.");
      ...
      for (int tChanIndex = 0; tChanIndex < tDecTable->GetChannelCount() + 1; tChanIndex++) {
        if (tDecTable->GetDecayChannel(tChanIndex)->Is3Particle()) {
          PRINT_DEBUG_2(
              "\t\tChannel "
              << tChanIndex << ": "
              << (aPartDB->GetParticleType(tDecTable->GetDecayChannel(tChanIndex)->GetParticle1()))
                     ->GetName()
              << " + "
              << (aPartDB->GetParticleType(tDecTable->GetDecayChannel(tChanIndex)->GetParticle2()))
                     ->GetName()
              << " + "
              << (aPartDB->GetParticleType(tDecTable->GetDecayChannel(tChanIndex)->GetParticle3()))
                     ->GetName()
 ...
 *   <tr><td>Randomize</td>     <td>all random generators are initiated with system date or have
 * constant seed </td></tr> <tr><td>FreezeOutDir</td>      <td>directory with freeze-out models
 * setting files</td></tr> <tr><td>FreezeOutModel</td>    <td>name of the freeze-out model to
 * use</td></tr>
 * </table>
 *
 ...
 * @brief Global variable with number of Monte-Carlo samples used by Integrator to find the average
 * multiplicity and max of the Cooper-Frye integrand for each ParticleType in the selected Model.
 *
 * @var int sParentPID
