...
Integrator::Integrator() : mFOModel(0), mNSamples(0) {}

Integrator::Integrator(int aNSamples, Model* FOModel) : mNSamples(aNSamples) {
  mFOModel = FOModel;
}
...
void Integrator::GenerateParticles(ParticleType* aPartType, int aPartCount,
  ...
  PRINT_DEBUG_3("Integrator::GenerateParticles\t "
                ...
                << " I3:" << aPartType->GetI3() << " S:" << aPartType->GetStrangeN()
                << " C:" << aPartType->GetCharmN());
...
bool Integrator::SetMultiplicities(ParticleDB* aDB, const TString& filename) {
  ...
  if ((tFileIn) && (tFileIn.is_open())) {
    ...
    PRINT_DEBUG_1(
        "<Integrator::SetMultiplicities>\tReading Max Integrand and Multiplicity values from "
        << tMultiName);
    ...
    if ((tFileOut) && (tFileOut.is_open())) {
      PRINT_DEBUG_1("<Integrator::SetMultiplicities>\tMax Integrand and Multiplicity file "
                    << tMultiName << " not found.");
      tDate.Set();
      ...
      for (int tIter = 0; tIter < aDB->GetParticleTypeCount(); tIter++) {
        ...
        tFileOut << aDB->GetParticleType(tIter)->GetName() << '\t' << tMaxInt << '\t' << tMulti
                 << endl;
        cout << "\r\tparticle (" << (tIter + 1) << "/" << aDB->GetParticleTypeCount()
             << "): " << aDB->GetParticleType(tIter)->GetName();
        cout.flush();
      }
      cout << endl;
      tFileOut.close();
      if (rename(tTempName, tMultiName) != 0) {
        PRINT_MESSAGE("<Integrator::SetMultiplicities>\tUnable to rename temp file to "
                      << tMultiName);
