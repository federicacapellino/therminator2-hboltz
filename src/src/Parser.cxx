...
Parser::Parser() : mFileName(""), mFile(0) {}

Parser::Parser(const char* aFileName) : mFileName(aFileName) {
    ...
    if ((*tDaughter3 > 65) && (*tDaughter3 < 122)) {
      ...
      *iss >> tBRatio >> CGcoeff;
      ...
      if (aDB->GetParticleType(tDaughter1)->GetMass() +
        ...
        PRINT_DEBUG_2("\t\tNOT adding 3-body decay channel: "
                      << tDaughter1 << " + " << tDaughter2 << " + " << tDaughter3
                      << ", extra mass = "
                      << aDB->GetParticleType(tDaughter1)->GetMass() +
                             aDB->GetParticleType(tDaughter2)->GetMass() +
                             aDB->GetParticleType(tDaughter3)->GetMass() -
                             aDB->GetParticleType(tFather)->GetMass());
    } else {
      ...
      *iss >> CGcoeff;
      ...
      if (CGcoeff) { // complete branching ratio by Clebsch-Gordan coefficient
        ...
        PRINT_DEBUG_3("\t\tCG coeff for: J(" << J << ") M(" << M << ") j1(" << j1 << ") m1(" << m1
                                             << ") j2(" << j2 << ") m2(" << m2 << ")");
        ...
        // and different I3?
        if ((TMath::Abs(aDB->GetParticleType(tDaughter1)->GetSpin() -
                        aDB->GetParticleType(tDaughter2)->GetSpin()) < 0.01) &&
            (TMath::Abs(aDB->GetParticleType(tDaughter1)->GetMass() -
                        aDB->GetParticleType(tDaughter2)->GetMass()) < 0.01) &&
            (TMath::Abs(aDB->GetParticleType(tDaughter1)->GetI3() -
                        aDB->GetParticleType(tDaughter2)->GetI3()) > 0.01) &&
            (aDB->GetParticleType(tDaughter1)->GetStrangeN() -
                 aDB->GetParticleType(tDaughter2)->GetStrangeN() ==
             0) &&
            (aDB->GetParticleType(tDaughter1)->GetCharmN() -
                 aDB->GetParticleType(tDaughter2)->GetCharmN() ==
             0)) {
      ...
      if (aDB->GetParticleType(tDaughter1)->GetMass() +
        ...
        PRINT_DEBUG_2("\t\tNOT adding 2-body decay channel: "
                      << tDaughter1 << " + " << tDaughter2 << ", extra mass = "
                      << aDB->GetParticleType(tDaughter1)->GetMass() +
                             aDB->GetParticleType(tDaughter2)->GetMass() -
                             aDB->GetParticleType(tFather)->GetMass());
    }
...
double Parser::SHAREClebschGordan(double aJot, double aEm, double aJot1, double aEm1, double aJot2,
                                  double aEm2) {
  int mint, maxt;
  double cgc = 0.0;
  int tIter;
  ...
  for (tIter = mint; tIter <= maxt; tIter++) {
    ...
    coef *= TMath::Sqrt((2 * aJot + 1) * factorials[lrint(aJot1 + aEm1)] *
                        factorials[lrint(aJot1 - aEm1)] * factorials[lrint(aJot2 + aEm2)] *
                        factorials[lrint(aJot2 - aEm2)] * factorials[lrint(aJot + aEm)] *
                        factorials[lrint(aJot - aEm)]);
    PRINT_DEBUG_3("\t\t\tcoef2 " << coef);
    coef /= factorials[tIter] * factorials[lrint(aJot1 + aJot2 - aJot - tIter)] *
            factorials[lrint(aJot1 - aEm1 - tIter)] * factorials[lrint(aJot2 + aEm2 - tIter)] *
...
double Parser::SHAREDeltaJ(double aJot1, double aJot2, double aJot) {
  double res = TMath::Sqrt(
      1.0 * factorials[lrint(aJot1 + aJot2 - aJot)] * factorials[lrint(aJot1 - aJot2 + aJot)] *
      factorials[lrint(-aJot1 + aJot2 + aJot)] / factorials[lrint(aJot1 + aJot2 + aJot + 1)]);
  return res;
}
  ...
  while (!mFile.eof()) {
    ...
    if (buff.BeginsWith("<?") && buff.EndsWith("?>")) {
      ...
      if (buff.CompareTo(aXML->mCurrTag->name))
        PRINT_MESSAGE("<Parser::ReadXML>\tClosing tag name mismatch \""
                      << aXML->mCurrTag->name << "\" != \"" << buff << "\"");
      PRINT_DEBUG_2("XML end tag     : </" << aXML->mCurrTag->name << "> @ "
                                           << aXML->mCurrTag->end);
      ...
      // TAG closed - go to parent
      aXML->mCurrTag = tTag;
      PRINT_DEBUG_2("XML empty tag   : <" << aXML->mCurrTag->name << " /> @ "
                                          << aXML->mCurrTag->begin);
      for (std::list<XMLattribute>::iterator Iter = aXML->mCurrTag->attributes.begin();
           Iter != aXML->mCurrTag->attributes.end(); Iter++)
      ...
      aXML->mCurrTag = tTag;
      PRINT_DEBUG_2("XML begin tag   : <" << aXML->mCurrTag->name << "> @ "
                                          << aXML->mCurrTag->begin);
      for (std::list<XMLattribute>::iterator Iter = aXML->mCurrTag->attributes.begin();
           Iter != aXML->mCurrTag->attributes.end(); Iter++)
        PRINT_DEBUG_3("  attribute     :  " << Iter->name << "=\"" << Iter->value << "\"");
