...
ParticleDecayer::ParticleDecayer() : mParticles(0), mDB(0), mTypeF(0), mFather(0), mChannel(0) {}
...
int ParticleDecayer::DecayParticle(Particle* aFather) {
  ...
  if (tChannelIndex == -1) {
    PRINT_DEBUG_3("<ParticleDecayer::DecayParticle>\tNot decaying " << mTypeF->GetName()
                                                                    << " for prob " << tProb);
    for (int tIter = 0; tIter <= tTable->GetChannelCount(); tIter++) {
      if (tTable->GetDecayChannel(tIter)->GetParticle3() > -1) {
        PRINT_DEBUG_3(
            "\tD1 "
            << mDB->GetParticleType(tTable->GetDecayChannel(tIter)->GetParticle1())->GetName()
            << " D2 "
            << mDB->GetParticleType(tTable->GetDecayChannel(tIter)->GetParticle2())->GetName()
            << " BR " << tTable->GetDecayChannel(tIter)->GetBranchingRatio());
      } else {
        PRINT_DEBUG_3(
            "\tD1 "
            << mDB->GetParticleType(tTable->GetDecayChannel(tIter)->GetParticle1())->GetName()
            ...
            << mDB->GetParticleType(tTable->GetDecayChannel(tIter)->GetParticle2())->GetName()
            << " D3 "
            << mDB->GetParticleType(tTable->GetDecayChannel(tIter)->GetParticle3())->GetName()
            << " BR " << tTable->GetDecayChannel(tIter)->GetBranchingRatio());
      }
    }
    return 0;
