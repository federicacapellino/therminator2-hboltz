...
double Thermodynamics::GetInvFugacityFactor(ParticleType* aPartType) const {
  ...
  case 0:
    return Exp(-GetChemicalPotential(aPartType) / GetTemperature());
  case 2:
    return 1.0 / GetFugacity(aPartType);
  default:
    return 1.0;
  }
}

double Thermodynamics::GetInvFugacityFactor(ParticleType* aPartType, double aX, double aY,
                                            double aZ) const {
  switch (GetChemistryType()) {
  case 1:
    return Exp(-GetChemicalPotential(aPartType, aX, aY, aZ) / GetTemperature(aX, aY, aZ));
  case 3:
    return 1.0 / GetFugacity(aPartType, aX, aY, aZ);
  default:
    return 1.0;
  }
}

double Thermodynamics::GetViscosityCorrection(double aX, double aY, double aZ, double aPe,
  ...
  case 0:
    ...
    break;
  case 1:
    tBulkPart = (tCs2 * aPdotU + (tMass2 - aPdotU * aPdotU) / (3.0 * aPdotU)) * GetBulkTau() *
                GetBulkPi(aX, aY, aZ) / tTemp;
    break;
  case 2:
    tShearPart = 1.0 / (2.0 * tTemp * tTemp) * GetShearPPdotPi(aX, aY, aZ, aPe, aPx, aPy, aPz);
    tBulkPart = (tCs2 * aPdotU + (tMass2 - aPdotU * aPdotU) / (3.0 * aPdotU)) * GetBulkTau() *
