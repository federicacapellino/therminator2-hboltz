...
Vector3D::Vector3D()
    : mVecName(""), mVec(0), mXmin(0.0), mXmax(1.0), mXpts(1), mYmin(0.0), mYmax(1.0), mYpts(1),
...
Vector3D::Vector3D(const char* aName, double aXmin, double aXmax, int aXpts, double aYmin,
                   double aYmax, int aYpts, double aZmin, double aZmax, int aZpts)
    : mVecName(aName), mXmin(aXmin), mXmax(aXmax), mXpts(aXpts), mYmin(aYmin), mYmax(aYmax),
      mYpts(aYpts), mZmin(aZmin), mZmax(aZmax), mZpts(aZpts) {
  if (mXpts <= 0)
    mXpts = 1;
  if (mYpts <= 0)
    mYpts = 1;
...
const char* Vector3D::GetName() const {
  return mVecName.Data();
}
double Vector3D::GetXMin() const {
  return mXmin;
}
double Vector3D::GetXMax() const {
  return mXmax;
}
int Vector3D::GetXPts() const {
  return mXpts;
}
double Vector3D::GetYMin() const {
  return mYmin;
}
double Vector3D::GetYMax() const {
  return mYmax;
}
int Vector3D::GetYPts() const {
  return mYpts;
}
double Vector3D::GetZMin() const {
  return mZmin;
}
double Vector3D::GetZMax() const {
  return mZmax;
}
int Vector3D::GetZPts() const {
  return mZpts;
}
...
double Vector3D::Interpolate1D(double aX) {
  ...
  j = 0;
  k = 0;
  return mVec[i][j][k] * (1 - ti) + mVec[i + 1][j][k] * ti;
}
...
double Vector3D::Interpolate2D(double aX, double aY) {
  ...
  tj -= j;
  k = 0;
  return (mVec[i][j][k] * (1 - ti) + mVec[i + 1][j][k] * ti) * (1 - tj) +
...
inline double Vector3D::Derivative(double aFin, double aFi0, double aFip) {
  return (mXin * (mXin - 2.0 * mX0) * (aFip - aFi0) + mXi0 * (mXi0 - 2.0 * mX0) * (aFin - aFip) +
