...
Viscosity::Viscosity() : mViscosityType(0), mBulk(0), mTau(0.0) {
  for (int i = 0; i < 10; i++)
    mShear[i] = 0;
}
...
double Viscosity::GetShearPPdotPi(double aX, double aY, double aZ, double aPe, double aPx,
                                  double aPy, double aPz) const {
  return (aPe * aPe * mShear[i00]->Interpolate(aX, aY, aZ) +
          aPx * aPx * mShear[iXX]->Interpolate(aX, aY, aZ) +
          aPy * aPy * mShear[iYY]->Interpolate(aX, aY, aZ) +
          aPz * aPz * mShear[iZZ]->Interpolate(aX, aY, aZ) +
          2.0 * (aPx * aPy * mShear[iXY]->Interpolate(aX, aY, aZ) +
